...
from data_generator import build_supported_target_dataset


def build_rubric() -> vf.Rubric:
    rubric = vf.Rubric(
        funcs=[
            backend.transition_target_satisfied_metric,
            backend.full_press_gate_metric,
        ],
        weights=[0.75, 0.25],
    )
...
def load_environment(
    ...
    **kwargs: Any,
    ...
    dataset = build_supported_target_dataset(num_sessions=num_train_examples, seed=seed)
    rubric = build_rubric()
