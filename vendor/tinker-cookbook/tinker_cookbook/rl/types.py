...
class SamplingRef:
    """
    Minimal, serializable reference for reconstructing the current sampling policy.

    This is used by rollout backends that cannot safely share a live
...
class EnvGroupBuilder(ABC):
    ...
    **Picklability:** Implementations must be pickleable (via standard ``pickle``) to support
    ...
    requires_in_process_rollout: bool = False
    ...
    async def run_group_rollout(
        ...
        *,
        ...
        """
        Optional extension point for custom group-rollout execution.

        Implementations can override this to route trajectories through a custom
        execution backend while still returning a standard ``TrajectoryGroup``.
        The default behavior is to defer to ``make_envs()`` and the local policy
        rollout logic in ``tinker_cookbook.rl.rollouts``.
        """
