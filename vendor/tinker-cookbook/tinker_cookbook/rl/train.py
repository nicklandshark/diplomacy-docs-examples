...
class Config:
    ...
    extra_metrics_provider: Callable[[], dict[str, float]] | None = None
...
def _builder_requires_in_process_rollout(env_group_builder: EnvGroupBuilder) -> bool:
    return bool(getattr(env_group_builder, "requires_in_process_rollout", False))


def _dataset_requires_in_process_rollout(dataset: RLDataset | None) -> bool:
    if dataset is None:
        return False
    for i_batch in range(len(dataset)):
        if any(_builder_requires_in_process_rollout(builder) for builder in dataset.get_batch(i_batch)):
            return True
    return False


def _get_extra_metrics(cfg: Config) -> dict[str, float]:
    if cfg.extra_metrics_provider is None:
        return {}
    metrics = cfg.extra_metrics_provider()
    return {
        key: float(value)
        for key, value in metrics.items()
...
def _log_metrics(
    ...
    *,
    ...
    combined_metrics.update(_get_extra_metrics(cfg))
    ml_logger.log_metrics(combined_metrics, step=step)


def _ensure_rollout_executor_compatible(
    ...
    if rollout_executor is None:
        return
    if _dataset_requires_in_process_rollout(dataset) or _dataset_requires_in_process_rollout(
        maybe_test_dataset
    ):
        raise ValueError(
            "Delegated rollout runners require in-process execution. "
            "Disable rollout_executor when using builders with requires_in_process_rollout=True."
        )
...
async def do_sync_training_with_stream_minibatch(
    ...
    for i_batch in range(start_batch, end_batch):
        ...
        _log_metrics(ml_logger, cfg, metrics, step=i_batch)
    ...
    async def training_loop():
        ...
        while i_batch < end_batch:
            ...
            if cfg.stream_minibatch_config is not None:
                ...
                sampling_client, sampling_ref, train_step_metrics = await do_train_step_and_get_sampling_client(
                    cfg,
                    i_batch,
                    training_client,
                    kl_reference_client,
    ...
    async def evaluation_loop():
        ...
        while not shutdown_event.is_set():
            ...
            if cfg.eval_every > 0 and sampling_client_eval_step % cfg.eval_every == 0:
                ...
                _log_metrics(ml_logger, cfg, metrics, step=sampling_client_eval_step)
...
async def save_checkpoint_and_get_sampling_client(
...
) -> tuple[tinker.SamplingClient, SamplingRef, dict[str, Any]]:
    metrics = {}
    with timed("save_checkpoint", metrics):
        ...
        metrics["sampling/saved_sampler"] = 1.0
        sampling_client = training_client.create_sampling_client(sampler_path)
        return sampling_client, _build_sampling_ref(cfg, sampler_path), metrics
...
async def compute_full_batch_metrics_and_get_sampling_client(
    ...
    sampling_client, sampling_ref, checkpoint_metrics = await save_checkpoint_and_get_sampling_client(
        cfg,
        training_client,
        i_batch,
        log_path,
    ...
    metrics["sampling/has_sampler_path"] = 1.0
    ...
    return sampling_client, sampling_ref, metrics
...
async def do_train_step_streaming_and_get_sampling_client(
...
) -> tuple[tinker.SamplingClient, SamplingRef, dict[str, Any], list[WrappedTrajectoryGroup]]:
    """
    ...
    metrics.update(full_batch_metrics)
    return sampling_client, sampling_ref, metrics, all_wrapped_trajectory_groups
...
async def do_train_step_and_get_sampling_client(
    ...
    sampling_client, sampling_ref, full_batch_metrics = await compute_full_batch_metrics_and_get_sampling_client(
        cfg,
        training_client,
        # NOTE: saving the checkpoint as the i + 1 step
        i_batch + 1,
...
async def main(
    ...
    try:
        ...
        _ensure_rollout_executor_compatible(rollout_executor, dataset, maybe_test_dataset)
