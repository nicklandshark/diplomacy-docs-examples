...
def _log_trajectory_group_details(trajectory_group: TrajectoryGroup) -> None:
...
async def do_group_rollout(
    ...
    trajectory_group = TrajectoryGroup(trajectories_G, list(rewards_G), list(metrics_G))
    _log_trajectory_group_details(trajectory_group)
...
async def _do_group_rollout_and_filter_constant_reward_impl(
    ...
    if type(env_group_builder).run_group_rollout is not EnvGroupBuilder.run_group_rollout:
        if sampling_ref is None:
            raise ValueError(
                "sampling_ref is required when EnvGroupBuilder.run_group_rollout() is overridden"
            )
        with logtree.optional_enable_logging(enable_logging):
            trajectory_group = await env_group_builder.run_group_rollout(
                sampling_ref,
                max_tokens=max_tokens,
                temperature=temperature,
                do_remove_constant_reward_groups=do_remove_constant_reward_groups,
                enable_logging=enable_logging,
            )
            if trajectory_group is not None:
                _log_trajectory_group_details(trajectory_group)
        if (
            trajectory_group is not None
            and do_remove_constant_reward_groups
            and all_same(trajectory_group.get_total_rewards())
        ):
            return None
        return trajectory_group
